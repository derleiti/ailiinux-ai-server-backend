...
Tests auto-publisher functionality including WordPress and bbPress integration.
...
class TestAutoPublisher:
    """Test suite for AutoPublisher with WordPress and bbPress integration."""
    ...
    @pytest.mark.asyncio
    async def test_create_forum_topic(self, auto_publisher):
        """
        Test creating bbPress forum topic from crawl result.

        Happy path: Verify forum topic creation.
        """
        result = CrawlResult(
            id="test-result",
            job_id="test-job",
            ...
            summary="Interesting discussion topic",
            headline=None,
            content="Content",
            excerpt="Excerpt",
            meta_description=None,
        ...
        mock_topic_result = {"id": 456}

        with patch('app.services.auto_publisher.bbpress_service') as mock_bbpress:
            with patch('app.services.auto_publisher.crawler_manager') as mock_crawler:
                mock_bbpress.create_topic = AsyncMock(return_value=mock_topic_result)
                mock_crawler._store.update = AsyncMock()
                ...
                call_args = mock_bbpress.create_topic.call_args
                assert call_args.kwargs["title"] == "Forum Topic Title"
                assert call_args.kwargs["forum_id"] == 3
                assert call_args.kwargs["tags"] == ["ai", "tech"]

    @pytest.mark.asyncio
    async def test_create_forum_topic_error_handling(self, auto_publisher):
        """
        Test forum topic creation error handling.

        Error condition: Verify errors are logged but don't crash.
        """
        result = CrawlResult(
            id="test-result",
            job_id="test-job",
        ...
        with patch('app.services.auto_publisher.bbpress_service') as mock_bbpress:
            mock_bbpress.create_topic = AsyncMock(side_effect=Exception("API Error"))
    ...
    async def test_process_hourly_filters_posted(self, auto_publisher):
        ...
        with patch('app.services.auto_publisher.crawler_manager') as mock_crawler:
            ...
            with patch.object(auto_publisher, '_create_wordpress_post', new=AsyncMock()):
                with patch.object(auto_publisher, '_create_forum_topic', new=AsyncMock()):
                    await auto_publisher._process_hourly()
