...
class TestCrawlerManager:
    ...
    async def test_job_prioritization(self, crawler_manager):
        ...
        # Mock the _run_worker to capture the order of job processing
        processed_jobs = []
        ...
        async def mock_run_worker():
            while True:
                job_id = None
                try:
                    job_id = await asyncio.wait_for(crawler_manager._high_priority_job_queue.get(), timeout=0.01)
                except asyncio.TimeoutError:
                    pass

                if job_id is None:
                    try:
                        job_id = await asyncio.wait_for(crawler_manager._job_queue.get(), timeout=0.01)
                    except asyncio.TimeoutError:
                ...
                if job_id:
                    ...
                    if job_id == high_priority_job.id:
                        crawler_manager._high_priority_job_queue.task_done()
                    else:
                        crawler_manager._job_queue.task_done()

        # Run the mocked worker for a short period
        worker_task = asyncio.create_task(mock_run_worker())
        await asyncio.sleep(0.1) # Give worker some time to process
        worker_task.cancel()
        try:
            await worker_task
        ...
        # Assert that the high-priority job was processed first
        assert len(processed_jobs) == 2
        assert processed_jobs[0] == high_priority_job.id
        assert processed_jobs[1] == low_priority_job.id
