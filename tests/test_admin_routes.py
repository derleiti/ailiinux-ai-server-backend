...
def test_get_crawler_config():
    response = client.get('/admin/crawler/config')
...
def test_update_crawler_config(monkeypatch, restore_settings):
    ...
    response = client.post('/admin/crawler/config', json=payload)
    assert response.status_code == 200
    data = response.json()
...
def test_control_user_start_idempotent(monkeypatch):
    ...
    response = client.post('/admin/crawler/control', json={'action': 'start', 'instance': 'user'})
    assert response.status_code == 200
    payload = response.json()
...
def test_control_user_stop(monkeypatch):
    ...
    response = client.post('/admin/crawler/control', json={'action': 'stop', 'instance': 'user'})
    assert response.status_code == 200
    payload = response.json()
